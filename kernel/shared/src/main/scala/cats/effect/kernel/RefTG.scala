...
object RefTG {
  ...
  private[kernel] trait MakeInstances extends MakeLowPriorityInstances {
    implicit def concurrentInstance[F[_]](implicit F: GenConcurrent[F, _]): Make[F] =
      new Make[F] {
        override def refOf[A](a: A): F[RefTG[F, A]] = ??? //F.ref(a)
      }
  }
  ...
  private[kernel] trait MakeLowPriorityInstances {
    implicit def syncInstance[F[_]](implicit F: SyncTG[F]): Make[F] =
      new Make[F] {
        override def refOf[A](a: A): F[RefTG[F, A]] = F.delay(unsafe(a))
      }
  }
   ...
   * }}}
   */
  def of[F[_], A](a: A)(implicit mk: Make[F]): F[RefTG[F, A]] = mk.refOf(a)
  ...
  def unsafe[F[_], A](a: A)(implicit F: SyncTG[F]): RefTG[F, A] =
    new SyncRefTG[F, A](new AtomicReference[A](a))
   ...
   * }}}
   */
  def lens[F[_], A, B <: AnyRef](ref: RefTG[F, A])(get: A => B, set: A => B => A)(
      implicit F: SyncTG[F]): RefTG[F, B] =
    new LensRefTG[F, A, B](ref)(get, set)
  ...
  final class ApplyBuilders[F[_]](val mk: Make[F]) extends AnyVal {
     ...
     * @see [[Ref.of]]
     */
    def of[A](a: A): F[RefTG[F, A]] = mk.refOf(a)
  }

  final private class SyncRefTG[F[_], A](ar: AtomicReference[A])(implicit F: SyncTG[F])
      extends RefTG[F, A] {
  ...
  implicit def catsInvariantForRef[F[_]: Functor]: Invariant[RefTG[F, *]] =
    new Invariant[RefTG[F, *]] {
      override def imap[A, B](fa: RefTG[F, A])(f: A => B)(g: B => A): RefTG[F, B] =
        new RefTG[F, B] {
